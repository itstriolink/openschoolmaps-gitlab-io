...
.Zeitplanung
Sie werden je nach Methode
und Anzahl Geodaten etwa 20-40min für dieses Arbeitsblatt brauchen.
...
können Sie mit dem übernächsten Kapitel
...
an,
wenn Sie nur wenige Geodaten erfassen müssen.

=== Geodaten erfassen

Anstatt die Daten in uMap selbst zu erfassen,
können Sie diese auch aus einer einfach zu erstellenden Datei einlesen.
Diese kann im textbasierten GeoJSON-Format vorliegen,
das Sie z.B.
auf der Website geojson.io
oder mit QGIS Desktop
erzeugen können.
(Für letzteres sehen Sie
https://www.qgistutorials.com/de/.)
...
dass Ihre Daten auch für OpenStreetMap geeignet sind.
Das heisst *keine* persönliche Daten
und *nur* objektive/überprüfbare Fakten
