...
.Zeitplanung
Je nach Grösse Ihrer Story-Map sollten Sie etwa 45 Minuten für diese Anleitung planen.
...
image::umap/story-map_erstellen/uMap_homepage.png[]
  * Tragen Sie Punkte image:umap/stecknadel_icon.PNG["Punkte-Icon", 25, 25] ein, die Sie gerne auf der Map hätten. Sie können auch Linien image:umap/weg_icon.PNG["Linien-Icon", 25, 25] und Flächen image:umap/flaeche_icon.PNG["Flächen-Icon", 25, 25] auf die Karte eintragen, wenn Sie möchten.
...
Wenn Sie daher eigene Bilder auf der Storymap darstellen möchten, müssen Sie die Bilder auf einer Online-Platform hochladen.
Eine Methode, mit der Sie gleichzeitig in OpenStreetMap mitwirken können, ist _Mapillary_. (https://mapillary.com)
Sie können auf der Webseite Bilder hochladen und mit einer Ortschaft in Verbindung setzen,
oder schon bestehende Bilder auf _Mapillary_ nutzen, und diese in uMap einzubinden.

Wenn Sie nur die bestehenden Bilder von _Mapillary_ verwenden möchten, müssen Sie lediglich ein Bild auswählen
und unter _Image options_ auf _Download image_ klicken.
...
Danach können Sie mit den gleichen Schritten, wie wenn Sie ein bestehendes Bild kopieren möchten, Ihr Bild in uMap einbinden.

NOTE: Ihre hochgeladenen Bilder werden vom Mapillary Team verifiziert.
Daher könnte es einige Studen dauern, bis Ihre Bilder auf der Webseite verfügbar sind.

Um nachzuschauen ob es schon Bilder in der Umgebung gibt, in welcher Sie ihren Punkt eingetragen haben,
können Sie http://projets.pavie.info/pic4carto/ brauchen.
...
Wenn Sie nun eine Bild-URL haben wie: `https://www.example.com/picture.png`, dann müssen Sie folgenden Text eingeben: `{{https://www.example.com/picture.png}}`
