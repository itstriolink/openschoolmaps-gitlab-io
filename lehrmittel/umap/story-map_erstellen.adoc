...
.Aim
The aim of this guide is to create a story map with uMap as part of the OpenSchoolMaps project. In geography, a story map provides an attractive and informative combination of publicly available maps, accompanying text, images and multimedia content.
...
.Target Group
For all who are interested in "Mapping".
...
image::umap/story-map_erstellen/uMap_homepage.png[]
  * Enter points image:umap/stecknadel_icon.PNG["Punkte-Icon", 25, 25] you would like to have on the map. You can also select the line image:umap/weg_icon.PNG["Linien-Icon", 25, 25] and area image:umap/flaeche_icon.PNG["Flächen-Icon", 25, 25].
* Now to the actual "story". The most important step is to activate the slide show under "Settings image:umap/story-map_erstellen/zahnrad_icon.png["Zahnrad-Icon", 25, 25] Slide show -> Activate slide show mode". After saving the settings, you probably need to refresh with F5.
...
To add a description, click (in edit mode) on the item you want to edit and write your text in the "Description" box.
...
* To make headlines you need `# titles`. The more `#` the smaller the heading.
...
To use these values from the table, you must insert this text under "Settings -> Standard interaction options -> Popup template":
