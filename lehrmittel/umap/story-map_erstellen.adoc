...
Um ein Bild in die Beschreibung hinzuzufügen, braucht man den Link zum Bild.

Dieses Bild muss auf einer Online-Plattform vorhanden sein.

Um nachzuschauen, ob es schon Bilder in der Umgebung gibt, können Sie http://projets.pavie.info/pic4carto/ verwenden.

Wenn Sie bestehende Bilder von _Mapillary_ verwenden möchten, müssen Sie lediglich eines auswählen und unter _Image options_ auf _Download image_ klicken. Ein neuer Tab mit dem Bild öffnet sich und Sie können die URL kopieren und in Ihre uMap-Karte einbinden.

Eine Methode, mit der Sie gleichzeitig eigene Bilder teilen und in OpenStreetMap mitwirken können, ist _Mapillary_ (https://mapillary.com). Sie können auf  _Mapillary_  Bilder an einem Ort selber hochladen oder schon bestehende Bilder nutzen, und diese dann z.B. in uMap einbinden. Um auf _Mapillary_ eigene Bilder hochzuladen, benötigen Sie ein Benutzerkonto. Nach dem Upload können Sie mit den gleichen Schritten oben Ihr Bild in uMap einbinden.

NOTE: Hochgeladene Bilder werden vom Mapillary Team verifiziert. Das kann einige Zeit dauern, bis die Bilder auf _Mapillary_ verfügbar sind.
