...
5. Another way is to click on the polyline image:umap/weg_icon.PNG["Polyline icon", 25, 25] It can be used to show how visitors get from the bus stop to the event, as seen below.

+
.Draw a polyline (e.g. a path) on uMap. Map data (C) OpenStreetMap Contributors
image::umap/lageplan_erstellen/weg_platzieren.PNG[]
6. Lastly, click on the polygon image:umap/flaeche_icon.PNG["Polygon icon", 25, 25]. We used the polygon to draw the area of the sports field.
