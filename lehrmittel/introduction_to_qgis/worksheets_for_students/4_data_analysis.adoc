...
image::einfuehrung_in_qgis/abfragefrage1.png["Query question1"]

And here is the corresponding result:

.Output for the search on local.ch
image::einfuehrung_in_qgis/ausgabefrage1.png["Output question1"]
...
image::einfuehrung_in_qgis/abfragefrage2.png["Query question2"]

The result is returned in text form:

.Output for the search on sbb.ch
image::einfuehrung_in_qgis/ausgabefrage2.png["Output question2"]
...
*table:* Name of the table, which contains the attributes needed for the 
query.
...
|LIKE
|Comparison of characters. The following characters can be used as placeholders:
%: Placeholder for any number of characters.
For example, LIKE "F%" for all entries that begin with the letter F.
_ : Placeholder for exactly one character.
E.g. LIKE "Me_er", if you are looking for a person and don't know if you want to 
use Meier or Meyer's last name.
For LIKE "M__er" it could also be Mayer.
...
(SELECT
FROM addresses
WHERE "name" = 'Müller' AND "first_name" = 'Peter' AND "place" = 'Bern')
....
====
endif::show_solutions[]
...
.Input in QGIS
image::einfuehrung_in_qgis/aufgabe4abfrage.png["Exercise 4 query", 612, 36]

Solution:

.The entries shown in QGIS after a query.
image::einfuehrung_in_qgis/aufgabe4ausgabe.png["Exercise 4 query", 629, 71]
====
endif::show_solutions[]
...
"_SELECT * FROM Seen WHERE max_depth > 100_" only the part "_max_depth > 100_" 
has to be typed into the query field.
...
_"height_masl" AND "area" > 20_
....
4 Seen
====
endif::show_solutions[]

In more demanding analyses you want to use the whole SQL language.
The field filter is not sufficient for this.
For your own SQL queries, you can use the _DB Manager_ in QGIS, which you find 
in the QGIS menu under _Database_.
Given that data is already loaded in the project, you can use 
_Select Virtual Layer_.
If you open it, you will see _Project Layer_ and below it you see the queryable
layers that are loaded into the project, i.e. in our case Seen (lakes) and 
Kantonsgrenzen (cantonal borders).
To create your own SQL query, select _Project layer_ and click at the top of the
SQL window icon.
A new, two-part dialog appears with an empty text field at the top and an 
output field at the bottom.
In the upper field, the cursor is positioned at line 1 and you can start there 
to write the SQL query.

The SQL query can be executed by clicking the button _Execute_. 
If the query was successful, the result is being displayed in the output field. 
If you select the checkbox _Load as new layer_ before executing the SQL query, 
the result will be loaded as a new layer.

image::einfuehrung_in_qgis/db_manager.png["DB Manager", 750, 348]

image:einfuehrung_in_qgis/ausrufezeichen.png[, 15, 15]
_Exercise 6_ +
_Formulate the same query as in the previous task with the where-clause, but 
now as a standalone SQL query. What is the SQL query?_
...
(SELECT 
FROM lakes
WHERE "height_masl" < 400 AND "area" > 20
)....
_(Result list with 4 lakes)_
====
endif::show_solutions[]
...
image:einfuehrung_in_qgis/ausrufezeichen.png[, 15, 15]
_Exercise 9_ +
_What do you type into the calculator?_
...
("density" > 200 AND "year_of_accession" > 1800 AND "year_of_accession" < 1900)
....
====
endif::show_solutions[]
...
*By comparing to the features from:* "Kantonsgrenzen" +
*Topological operation:* "innerhalb"
...
WHERE "inhabitants" / "area" > 200 
AND "year_of_accession" < 1600 
AND "name" NOT LIKE 'Z%')
....
====
endif::show_solutions[]
...
When answering this question, only the lakes stored in the dataset  
qgis_einfuehrung_daten.gpkg must be considered. Perform the query and write down 
step by step how you proceed.  Small hint: Think about the task from the 
exercise, in which the names of all main locations were selected, whose 
associated cantons had to fulfill certain thematic conditions.
...
(SELECT * 
FROM cantonal_boundaries, lakes 
WHERE cantonal_boundaries.year_of_accession > 1600 AND lakes.area > 50 
AND st_intersects(lakes.geometry, cantonal_boundaries.geometry) )
....
====
endif::show_solutions[]
