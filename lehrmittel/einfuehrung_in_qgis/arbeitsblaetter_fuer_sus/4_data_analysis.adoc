...
SELECT
FROM Seen (lakes)
WHERE "Hoehe_muM" (height m.a.s.l) < 400 AND "Flaeche" (area) > 20
....
4 Seen
====
endif::show_solutions[]
...
====
.Solution
to_int (Einwohner (inhabitants) / Flaeche (area))
====
endif::show_solutions[]
...
.Solution
....
WHERE Dichte (density) > 200 AND Beitrittsj (year of accession) > 1800 AND Beitrittsj (year of accession) < 1900
....
====
endif::show_solutions[]
...
.Solution
SELECT * +
FROM Kantonsgrenzen (cantonal boundaries) +
WHERE Einwohner (inhabitants) / Flaeche (area) > 200 AND Beitrittsj 
(year of accession) < 1600 AND Name (name) NOT LIKE 'Z%'
====
endif::show_solutions[]
