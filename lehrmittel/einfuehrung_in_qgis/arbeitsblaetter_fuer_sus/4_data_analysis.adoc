...
SELECT
FROM Seen (lakes)
WHERE Flaeche (area) > 20 AND Tiefe (depth) < 400
....

.Solution 2
....
SELECT
FROM Adressen (addresses)
WHERE Name (name) = 'Müller' AND Vorname (first name) = 'Peter' AND Ort (place) = 'Bern'
....
====
endif::show_solutions[]
...
When the query is ready, you can execute it by clicking the button
_Execute_.
...
....
SELECT
FROM Seen --lakes, height m.a.s.l, area
...
When answering this question, only the lakes stored in the geopackage 
qgis_einfuehrung_daten.gpkg must be considered. Perform the query and write down 
step by step how you proceed.  Small hint: Think of the task from the exercise, 
in which the names of all main locations were selected, whose associated cantons
had to fulfill certain thematic conditions.
...
WHERE cantonal_boundaries.year_of_accession > 1600 +
AND lakes.area > 50 +
