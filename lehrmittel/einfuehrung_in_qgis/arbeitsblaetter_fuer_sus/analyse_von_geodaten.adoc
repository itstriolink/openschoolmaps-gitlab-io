...
displayed in a GIS. Thus, everything is prepared for the "speciality" of GIS, 
...
_As introduction for this chapter solve the following exercises:_
...
. Search 2: Next, go to the website http://www.sbb.ch/ and search for the next 
connection between the nearest train station and Chur. 
Again, pay attention to the output and input values. 
The result isn't as important as the format. 
Add these observations to your notes.
...
fields. The query could be like looks like:
...
second search only returns it in text form.
...
he's in, you can search for this visually.

For the timetable information the first thing you have to do, is departure and 
arrival times. The map is not included in the result being of lower 
importance.
====
endif::show_solutions[]

Keep your solutions in your my mind. We will come back to them later in 
this chapter.
...
. Queries based on factual data. 
Example: Select all lakes that have a water depth, that's lower than 100 meters. 
These queries are thematic queries or selections.

. Queries based on relationships between different geometric objects. 
Example: Select all lakes, which are completely within a canton. 
These queries are topological queries or selections.

. Queries via the geometry of the objects. 
Example: Which main locations are within a distance of less than 20 km around 
Lake Lucerne. These queries are geometric queries or selections.
...
next step is a simple update. For all others it's a short introduction into 
...
result of the query. 
* means, all attributes are displayed. 
For thematic queries in a GIS this is the only query possible. 
Therefore, this variable must not be set in the GIS.
...
. _The search that you did as introduction in the Search 1 at homepage of _
