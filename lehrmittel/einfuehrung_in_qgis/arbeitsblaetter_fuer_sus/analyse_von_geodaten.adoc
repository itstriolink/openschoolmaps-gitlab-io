...
Bei thematischen Abfragen in einem GIS ist nur die Abfrage nach allen 
Attributen möglich.
Deshalb muss diese Variable im GIS nicht eingestellt werden.
...
image:einfuehrung_in_qgis/ausrufezeichen.png[, 15, 15]
_Aufgabe 12_ +
_Welcher Layer ist denn nun der *Select features from*-Layer
und welcher der *By comparing to the features from*-Layer?
Welche Topologische Operation muss gewählt werden?_

ifdef::show_solutions[]
====
.Lösung
*Select features from:* Layer "Hauptorte" +
*By comparing to the features from:* Layer "Kantonsgrenzen" +
