...
====
Abfrage: +
Die Daten werden bei einer
Abfrage nicht verändert,
sondern lediglich bezogen.

Beispielsweise möchte man
wissen, wie viele Einwohner
einer Stadt mehr als 150‘000
CHF pro Jahr verdienen und
gleichzeitig jünger als 30
Jahre alt sind.

Manipulation: +
Die Daten werden bei einer
Manipulation verändert,
ergänzt oder sogar neue
Daten erzeugt.

Beispielsweise führt man
in ein bestehendes
Verkehrssystem ein neues
Attribut ein, ob eine Strecke
stark belastet ist oder nicht.
Oder eine weitere Möglichkeit
wäre auch das Einfügen von noch
...
SELECT * +
FROM Kantone +
WHERE  Einwohnerdichte > 200
AND Beitritt < 1600 AND
Name NOT LIKE ‘Z%’
====
endif::show_solutions[]
