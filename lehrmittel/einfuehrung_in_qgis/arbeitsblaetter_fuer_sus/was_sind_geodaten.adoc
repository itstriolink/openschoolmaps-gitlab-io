...
GIS dargestellt werden, um raumbezogene Daten handelt. Diese raumbezogenen 
Daten werden oft auch Geodaten genannt.
...
von direktem Raumbezug. Im Bereich der amtlichen Statistik hingegen müssen die 
...
Grenzen wie die Kantonsgrenzen verknüpft. In so einem Fall wird von indirektem 
Raumbezug gesprochen.
...
zugeordnet werden. Dieser Vorgang wird "georeferenzieren" genannt. Bei 
Postaddressen kann der Raumbezug durch "geocodieren" zugeordnet werden. Der 
Raumbezug bezieht sich natürlich immer auf eine Erdabbildung, d.h. auf ein 
Koordinatensystem. Diese Koordinatensysteme wurden eingeführt, um die 
geographische Lage eines Ortes auf der Erde in Zahlen angeben zu können. Die 
verschiedenen Koordinatensysteme kennst du bestimmt noch aus dem 
Geografieunterricht. Falls du dein Wissen in diesem Bereich wieder etwas 
auffrischen möchtest, kannst du dies mit Hilfe des Leitprogramms "Karten" tun. 
...
Lawinenforschung (SLF) an: https://www.slf.ch/de.html

===== Vektordaten

Zu den Vektordaten gehören z.B. die Shapefiles bzw. die Geopackage-Dateien, 
die du in der ersten Übung verwendet hast. Bei Vektordaten wird die Geometrie 
der Objekte als Punkte, Linien oder Flächen gespeichert, wobei eine Linie als 
Verbindung zweier Punkte und eine Fläche als geschlossener Linienzug definiert 
ist. Die weiteren Informationen zum Objekt, sogenannte Sachdaten, werden 
...
suchen. Hierzu kann QGIS, in Echtzeit das KBS für ein Projekt projizieren.
...
klicke auf den Button "Neues Projekt". In das leere Projekt sollen nun 
Geodaten geladen werden.

Bevor jedoch Daten geladen werden, sollte das Koordinatensystem des Projektes 
definiert werden. Dazu drückst du rechts unten in QGIS auf die Schaltfläche 
_Aktuelles KBS_. Mit dem Filter kannst du dann das gewünschte 
...
den OK-Button. Damit die neue Hintergrundfarbe erscheint, muss im 
...
wird das Menu Datenbank in der Menuleiste verwendet. Für die unterschiedlichen 
