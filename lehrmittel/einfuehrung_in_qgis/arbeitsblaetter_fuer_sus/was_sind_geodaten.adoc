...
kurzen Beschrieb, was genau in den Daten abgespeichert ist. Zusätzlich sind 
auch Informationen über die Ausdehnung und die möglichen Koordinatensysteme 
erhältlich.
...
du, dass bereits einige Server standardmässig eingebunden sind. Um den Server 
...
muss auf den _New_-Button geklickt werden. Gib im Fenster _Create a New 
WMS/WMTS Connection_ den Namen der Verbindung an (z.B. WMS-BGDI) und die URL 
zum gewünschten Server (in unserem Fall http://wms.geo.admin.ch/). 
...
====
.Lösung
Das Rasterformat speichert die Raster-Punkte, über die die Mauer läuft. + Das 
Vektorformat speichert Anfang- und Endpunkt der Mauer und den Punkt, an dem 
