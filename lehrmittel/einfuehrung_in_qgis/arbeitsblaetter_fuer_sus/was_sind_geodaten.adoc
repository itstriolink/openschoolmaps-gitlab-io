...
Falls du dein Wissen
in diesem Bereich
wieder etwas auffrischen möchtest,
kannst du dies mit Hilfe
des Leitprogramms
„Karten“ tun.
Dieses Leitprogramm ist
auf dem EducETH-Server
unter dem Link
...
====
.Lösung
. Wenn du weiter ins Bild reinzoomst, dann
siehst du ab einer gewissen Zoomstufe, dass
das Bild aus vielen farbigen Rechtecken besteht.
Je weiter du reinzoomst, desto grösser
werden diese Rechtecke.
. Ein Objekt mit Hilfe von vielen
farbigen Rechtecken abgespeichert.
====
endif::show_solutions[]
...
*Speicherplatzbedarf* +
Bei den Rasterdaten wird für jedes Pixel
ein Wert abgespeichert. Bei den Vektordaten
wird die Fläche des Gebäudes als geschlossener
Linienzug abgespeichert. Und jede Linie dieses
Linienzuges wird durch zwei Punkte definiert.
Deshalb ist der Speicherbedarf bei den
Vektordaten viel kleiner.

Damit der Speicherbedarf bei den
Rasterdaten kleiner wird, können
Formate verwendet werden, welche
die Raster komprimieren. Dabei wird
zwischen verlustbehafteten (z.B. jpg)
und verlustfreien Kompressionen
(z.B. .png, .bmp) unterschieden.
...
NOTE: Das KBS
(Koordinatenbezugssystem) sollte
in unserem vorgegebenen Projekt
schon richtig gesetzt sein.
Wenn du trotzdem wissen willst,
wie du das KBS ändern kannst,
kannst du das folgende
optionale Kapitel lesen.
...
QGIS hat einige nützliche
Werkzeuge, um nach spezifischen
Projektionen zu suchen.
Hierzu kann QGIS,
in Echtzeit
das KBS
für ein Projekt
...
Um das KBS
eines Projekts zu ändern,
kannst du
in der Statusleiste
auf den EPSG-Code klicken
und ein neues KBS auswählen.
...
ifdef::show_solutions[]
====
Rasterformat speichert die Punkte über
die die Mauer läuft. +
Vektorformat speichert Parameter woraus
es die Mauer darstellen kann
...
ifdef::show_solutions[]
====
Geodaten sind Daten die sich auf die Erde beziehen.
====
endif::show_solutions[]
...
ifdef::show_solutions[]
====
Format: Das Vektor-Layer nutzt Vektordaten
im Gegensatz zu den Rasterdaten des WMS-Layers. +
Speicherort: Das Vektor-Layer basiert auf
lokal gespeicherte Daten, während das WMS-Layer
sich auf Daten von Servern bezieht. +
Verfügbarkeit: WMS-Layerläuft über einen Server
und hat deshalb eine schlechter Verfügbarkeit
als das Vektor-Layer. Auch sendet der Server die
