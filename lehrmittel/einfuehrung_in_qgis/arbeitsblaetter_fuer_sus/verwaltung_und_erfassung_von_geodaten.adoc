= Introduction to QGIS 3: Erfassung und Verwaltung von Daten (Kapitel 3)
...
Die Zeile verläuft _horizontal_
und in ihr werden
die verschiedenen Datensätze
gespeichert.
Die Spalte verläuft _vertikal_
und in ihr wird
...
====
.Lösung
Vergleiche deine Lösung mit den
Datenerfassungmöglichkeiten aus der Theorie.
====
...
Die Lage der Hauptorte ist durch die Angabe
der X-Koordinate (XKoordinate) und der
Y-Koordinate (YKoordinate) angegeben. +

In der ersten Zeile des Textfiles sind
die Attributnamen abgespeichert. Diese Werte
sind in unserem Fall durch Kommas getrennt. +

Ab der 2. Zeile folgen die erfassten
Objekte. Die Attribte der einzelnen
Objekte sind ebenfalls durch Kommas
getrennt. Ein neues Objekt beginnt immer
auf einer neuen Zeile. Bei Textwerten
müssen zu Beginn und am Schluss des
Wertes Anführungszeichen stehen. +

In der ersten Zeile wird definiert,
welche Trennungszeichen im Dokument
verwendet werden. Die Zeichen die dort
gewählt wurden, müssen auch im weiteren
