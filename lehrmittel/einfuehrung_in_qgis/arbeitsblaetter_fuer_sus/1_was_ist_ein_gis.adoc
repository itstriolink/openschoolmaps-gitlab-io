...
sich bestimmt um ein Modell, welches auch bei Interdiscount oder Mediamarkt 
...
Dieses Projekt wollen wir jetzt laden. Klicke dazu auf den Button mit dem 
Namen Projekt öffnen und navigiere zum Ordner, in welchem du die Daten zu 
diesem Leitprogramm abgespeichert hast. Wähle die Datei Teil1.qgs aus und 
bestätige deine Eingabe durch Klicken auf den Open-Button.
...
du auf einen Layer rechtsklicken und _Zoom to Layer_ wählen. Nun solltest du 
die Karte im Kartenfenster sehen.

Die Darstellungsreihenfolge im Kartenfenster entspricht der Reihenfolge im 
Layerfenster. D.h. die Seen liegen im Layerfenser oberhalb der Kantonsgrenzen 
und werden deshalb auch im Kartenfenster über diesen angezeigt.
...
Maustaste an die neue Stelle. Probiere diese Funktion aus, indem du die 
Kantonsgrenzen im Layerfenster über die Seen legst. Beobachte dabei, was mit 
der Darstellung im Kartenfenster geschieht.
...
Hintergrundbild für das Bedienfeld _Overview_ wählen. Die Kantonsgrenzen 
...
und setze ein Kreuz bei der Option _Show in Overview_ anzeigen. Die 
