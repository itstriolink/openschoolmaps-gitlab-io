...
To help understand what a Geographic Information System is, we have here a definition for it:

.Definition
[NOTE]
====
A Geographic Information System (GIS) is a computersupported System, which is 
made out of Hardware, Software and Data and with witch one can model and 
modify spatial problems in diffrent fields of application.
The data/information required for this can be digitaly edited, managed, 
reorganized, analysed and presented both alphanumerically and graphically.
GIS denominates a technology as well as both a product and a means to access 
and modify geographic information.
...
._Chalkin created the Fourcomponent Model in the year 1977. He described the 
function of a GIS with 4 components Input, Manipulation, Analysis and 
Presentation, short IMAP. Exercise: Order each of these components to one of 
the functions mentioned in the definition._
...
We know to get deeper into the individual systemcomponents _Hardware, Software 
and Data_ and tell you something more about them.
...
choice is dependant on your operating system, since not every GIS supports 
each one.

The common GIS products can be devided into two classes. On one hand 
...
modify and spread of the software. Access to the sourcecode is unrestriced, 
...
In the first part of the theory you've learned that software is one of four components of the GIS system. In this exercise part you now learn how to use QGIS.
...
When you installed the QGIS software, it automaticaly created a folder with 
shortcuts for the programm on your desktop.
You can double click on that shortcut (named QGIS + version number) to start 
the programm. Doing that opens a simple graphical user interface.
...
The user interface is seperated into seven areas:
...
and and options menu opens up.

.List with tools
image::einfuehrung_in_qgis/qgis_werkzeugliste.png["Werkzeuge hinzufügen", 190, 266]

Try out these three methods und find out which one you like the most, for use in the future.

Hide the window "Spatial Bookmarks" and show the window "Overview" instead.

=== Open existing project

In this task we will work with an existing QGIS project. 
But first we'll need to load the project into our QGIS.
Klick for that on the button _Open Project_ and navigate to where you saved 
the data for this tutorial. Select the file Teil1.qgs and click on the "Open" button.

In the Layers window you should now be able to see two entries, Lakes and 
Canton borders. These entries are called layers and are shown graphicaly in 
the map window.
...
TIP: If you can't see the lake or cantons boarders you can right-cklick the 
...
You can change the order of the layers. 
Simply click on the the layer and move it up or down, try it with the lake 
layer and look what happens.
...
image:einfuehrung_in_qgis/ausrufezeichen.png[, 15, 15]
_Exercise 3_ +
_Try out the diffrent tools of the toolbox map navigation and write down in 
your own words what each one does, pay attention to what happens in the 
overview as well._

NOTE: For the navigation in the map window you can use mouse weel and keyboard 
as well. 
...
* _Move the mouse weel forward and backwards while the cursor is on the map._
...
During the exercise you've learnd some functions to adapt the view in the map window to your requirements.
...
._In which class and in which specific part of the class would the use of a 
GIS make sense?_ +
