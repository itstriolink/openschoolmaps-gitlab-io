...
100_" muss also nur der Teil "_max_Tiefe > 100_" ins Abfragefeld eingetippt 
werden.
...
Für eigene _SQL_-Anfragen kann man in QGIS den _DB Manager_ nutzen, den man 
im QGIS-Menu unter _Datenbank_ findet.
Gegeben, dass im Projekt schon Daten geladen sind, kann man dort  
_Virtuelle Layer_ wählen. 
Wenn man dort aufklappt, sieht man "Projektlayer" und darunter die im Projekt 
abfragbaren Layer, d.h. in unserem Falle Seen und Kantonsgrenzen.
Um eine eigene SQL-Anfrage durchzuführen, wählt man _Projektlayer_ und klickt 
oben auf das Icon _SQL-Fenster_. 
Es erscheint nun ein neuer, zweigeteilten Dialog mit einem leeren Textfeld oben 
und einem Ausgabefeld unten. 
Im oberen Feld steht der Cursor steht bei Zeile 1 und man kann dort SQL 
schreiben.
Wenn die SQL-Abfrage bereit ist, führt man sie mit Klick auf den Button 
_Ausführen_ aus.
...
nun aber als eigenständige SQL-Abfrage. Wie lautet die SQL-Anfrage?_
...
Der _Eingabelayer_ Layer in der Maske ist der Layer, in welchem die Elemente 
...
immer vom _Eingabelayer_ aus und überprüft, wie er im Verhältnis zum _Layer 
verknüpfen_ steht.
...
Option _Feldrechner öffen_. Falls du den Dichtewert in eine bereits 
bestehende Spalte schreiben möchtest, kannst du in der Maske _Forhandenes Feld erneuern_ 
ankreuzen und im Dropdown-Menu deine gewünschte Spalte auswählen. In 
unserem Fall möchten wir jedoch ein neues Feld anlegen. Gib deshalb unter 
_Ausgabefeld Name_ den Begriff _Dichte_ ein. Wähle als _Output field type 
Ganzzahl (integer)_ und als _Ausgabefeldlänge 10_.
...
Nun kannst du den _Feldrechner_ entsprechend der Formel für 
...
berechnen. Deshalb muss die Option zu _Ganzzahl_ gewählt werden.
...
Kantone und selektiere mit Hilfe der _Objekt über Ausdruck wählen_ alle 
...
SELECT * +
FROM Kantonsgrenzen +
WHERE Einwohner / Flaeche > 200 AND Beitrittsj < 1600 AND Name NOT LIKE 'Z%'
====
endif::show_solutions[]
...
AND st_intersects(Seen.geometry, Kantonsgrenzen.geometry)
====
endif::show_solutions[]
