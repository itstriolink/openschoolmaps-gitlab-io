...
Grosseltern). Klicke anschliessend auf den _Suchen_-Button. Was ist das 
...
komplizierter. Öffne durch Klicken auf den Button _Objekte über Ausdruck wählen_
...
werden. Von "_SELECT * FROM Seen WHERE max_Tiefe > 100_" muss also nur der Teil 
"_max_Tiefe > 100_" ins Abfragefeld eingetippt werden.
...
Für diese Berechnung muss die Attributstabelle der Kantone geöffnet werden. Wähle die 
Option _Feldrechner öffnen_. Falls du den Dichtewert in eine bereits 
bestehende Spalte schreiben möchtest, kannst du in der Maske _Forhandenes Feld erneuern_ 
ankreuzen und im Dropdown-Menü deine gewünschte Spalte auswählen. In 
unserem Fall möchten wir jedoch ein neues Feld anlegen. Gib deshalb unter 
_Ausgabefeld Name_ den Begriff _Dichte_ ein. Wähle als _Ausgabefeldtyp 
...
berechnen möchtest, musst du darauf achten, dass das Kreuz bei _Nur x ausgewählte
Objekte aktualisieren_ nicht gesetzt ist.
