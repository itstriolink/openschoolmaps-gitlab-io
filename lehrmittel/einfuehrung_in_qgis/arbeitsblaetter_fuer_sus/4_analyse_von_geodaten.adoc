...
Es ist auch möglich die ganze _SQL_-Abfrage zu nutzen. Dazu reicht aber der 
_Feldfilter_ leider nicht aus. Stattdessen nutzen wir dafür den _DB Manager_ 
welchen man unter _Datenbank_ findet.
Unter _Datenanbieter_ wählen wir nun aus was für einen Typ Datenbank wir 
nutzen, in unserem Beispiel verwenden wir _GeoPackage _ und die Datenbank 
+qgis_einfuehrung_daten.gpkg + und die Tabelle _Seen_.
Um eine SQL-Anfrage durchzuführen, nutzt man _SQL-fenster _ und erfasst es im 
Fenster oben rechts und führt es mit _Ausführen_ aus.
