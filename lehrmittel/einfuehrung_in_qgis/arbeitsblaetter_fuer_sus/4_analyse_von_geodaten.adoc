...
Es ist möglich auch die ganze _SQL_-Abfrage zu nutzen, dazu reicht aber der _Feldfilter_ nicht aus.
Stadessen nutzen wir dafür den _DB Manager_ welchen man unter _Datenbank_ findet.
Unter _Datenanbieter_ wählen wir nun aus was für eine art Datenbank wir nutzen, in unserem beispiel verwenden wir _GeoPackage_
Darunter wählt man die Datenbank +qgis_einfuehrung_daten.gpkg+ and am ende die Tabelle _Seen_ um sich zu verbinden.
Um eine SQL-Anfrage durchzuführen, nutzt man _SQL-fenster_ und erfasst es.
...
image::einfuehrung_in_qgis/db_manager.png["Frage 3 DB Manager", 750, 348]
