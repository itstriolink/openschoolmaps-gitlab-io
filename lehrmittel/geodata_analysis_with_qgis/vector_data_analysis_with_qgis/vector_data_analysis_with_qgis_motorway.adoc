...
.Map display of the analysis result. See also <<endergebnis-image>>
...
* Introduction to geodata processing
...
see https://de.wikipedia.org/wiki/Richtlinie_92/43/EWG_(Fauna-Flora-Habitat-Richtlinie)[Wikipedia])
...
=== Task 5: Edit the attribute table of the vector file `alle_zonen.shp`.
...
10. Create a map layout and export it as PDF. 
    (Before doing this, activate "`Print as Raster`" under General)
...
[[endergebnis-image]]
.Representation of the analysis result in an example layout.
