= Practice/Exercise: Spatial Analysis Raster Data - Where The Chamois Live
...
.Image CC BY-SA 4.0 Paul Hermans. (Source: https://commons.wikimedia.org/wiki/File:Gemse01.JPG)
[caption=""]
...
Only "`Channel 1 (grey)`" can be selected as the channel.
...
The main thing is that the cardinal directions
are now visualized by different colors.

image::02_perspective_final.png[]

== Slope map
With the function "`slope`" (menu menu:Raster[Analysis > Slope])
...
4. Areas with the use of meadow and arable land, 
   home pastures, spring pastures (Ger. "Maiensässe"), hay alps, 
...
Cutting and filtering is done with the "raster algebra" and in QGIS with the raster calculator.
...
For this you have to open the raster calculator via menu:Raster[].

image::04_rasterrechner_menu.png[]

In the raster calculator you have to enter the output layer in the upper right corner.
...
Double-click it to add it to the raster calculator expression in quotation marks.
...
Start the raster calculator with btn:[OK].
...
The same steps - first raster calculator then transparency
...
(`aspect.tif`) and filters out all raster cells that lie
...
Meadow and farmland, home pastures, spring pastures (Ger. "Maiensässe"), 
...
a singleband pseudocolor or palette:
...
(singleband pseudocolors copied from "perspective")
...
Here is the solution for the raster calculator expression:
[source,sql]
----
