...
== Introduction
A lot of real world objects can be displayed as points. These points are solo objects in the world that have a coordinate (usually lat/lon). It is possible to distinguish and identify a point solely by it's latitude and longitude, but we can not say the same for other types of objects as they can consist multiple points (which all have their own coordinate). (TBC)

.A points layer in QGIS
image::rendering_points_mit_qgis/qgis_points.png[QGIS and a points layer, 550]
...
image::rendering_points_mit_qgis/geojson_example.png[A .geojson file example, 550]
...
* QGIS environment (tested with QGIS 3.10.1-A Coruña)
* Data:
** `castles.geojson` file


After those has been achieved, we will load the necessary data into QGIS and start using and explaining some of the point rendering techniques offered by QGIS.
...
image::rendering_points_mit_qgis/point_displacement_map.png[Point displacement technique, 550]

With point displacement renderer, few options are displayed.
With these options, we can decide the:
...
image::rendering_points_mit_qgis/point_displacement_options.png[Point displacement options, 550]
...
image::rendering_points_mit_qgis/heatmap_generation_map.png[Heatmap generation technique, 550]
...
image::rendering_points_mit_qgis/heatmap_generation_options.png[Heatmap generation options, 550]
