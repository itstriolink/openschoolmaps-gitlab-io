...
This worksheet shows how you can use specific point rendering techniques in order to overcome map challenges involving points. We will be using different point renderers offered by QGIS in order to demonstrate how such challenges can be obstacled.
...
.A points layer on top of a map in QGIS
...
NOTE: To view the query for the OSM data in the _.geojson_ file using Overpass Turbo, you can just use the following link: https://osm.li/3FH
...
More questions? Contact OpenStreetMap Switzerland (info@osm.ch) or directly Prof. Stefan Keller (stefan.keller@ost.ch)!