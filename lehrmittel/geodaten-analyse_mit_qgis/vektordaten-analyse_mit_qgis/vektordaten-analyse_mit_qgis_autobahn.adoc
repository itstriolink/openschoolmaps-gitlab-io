...
klicke auf den Knopf "…" neben dem Feld für den Layername
und wähle "Speichern als Geopackage…".
Nenne die neue Datei autobahn_analyse.gpkg
und den neuen Layer trasse_20.

8. Führe die Verarbeitung mit dem Knopf "Starte" durch.
+
...
6. Schliesse das Layereigenschaften-Fenster mit «OK».
...
Bestätige mit OK und überprüfe,
...
Anschliessend solltest du deine neu erstellten Geometrien
als aussagekräftige Karten darstellen.
Du solltest die wertvollen Biotoptypen
in den Beeinträchtigungszonen oder die Konfliktzonen als Karte darstellen.

1. Klassifiziere Layer intersect_wertvoll nach Attribut ffh_typ_text.
...
6. Lade den neuen Layer und klassifizieren Ihn nach bfn_biotop_text
