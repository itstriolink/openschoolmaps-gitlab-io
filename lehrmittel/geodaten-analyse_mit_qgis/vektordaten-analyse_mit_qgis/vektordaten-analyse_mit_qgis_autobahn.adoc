...
zur Verfügung.
Weiterhin können Sie im Ordner
...
Sie sollen ermitteln, wie gross
...
1. Öffnen Sie QGIS
   und stellen Sie sicher, dass kein bestehendes Projekt geöffnet ist.
...
4. Öffnen Sie über Menü „Projekt“ > „Eigenschaften…“ die Projekteinstellungen,
   geben Sie dem Projekt unter Reiter „Allgemein“
...
1. Klassifizieren Sie Layer „umgebung “
...
2. Erstellen Sie ein Kartenlayout
   und exportieren Sie es als biotoptypen.pdf.
   Wenn Sie Texturen verwenden, sollten Sie vor dem Export
...
Nennen sie die neue Datei autobahn_analyse.gpkg
...
auf die Sie z.B. über das Kontext-Menü
des Layer-Eintrags im Layer-Panel gelangen.
...
verfügen Sie über den Autobahnkörper
...
Falls alles geklappt hat, können Sie die beiden Temporär-Layer
...
Vektor > Datenmanagement-Werkzeuge den Eintrag "Vektorlayer Zusammenführen…"
...
1. Öffnen Sie das Kontext-Menü des Laysers "zonen"
...
4. Stellen Sie mit Hilfe der Schaltflächen im Abfrageeditor
eine Abfrage zusammen, mit der Sie alle Flächen auswählen, die entweder
...
____
Öffnen Sie die QGIS-Optionen über Menü "Einstellungen" > "Optionen…"
und ändern Sie unter Reiter "Verarbeitung" > Abschnitt "Allgemein"
...
Verwenden Sie folgenden Ausdruck, um mit QGIS
...
Bestätigen Sie mit OK und überprüfen Sie,
...
11. Speichern Sie die neue Spalte, in dem Sie
den Bearbeitungsmodus für den Layer intersect_wertvoll ausschalten.


=== Aufgabe 8: Vektordateien klassifizieren und Karten erstellen

Anschliessend sollen Sie Ihre neu erstellten Geometrien
als aussagekräftige Karten darstellen.
Sie sollen die wertvollen Biotoptypen
in den Beeinträchtigungszonen oder die Konfliktzonen als Karte darstellen.

1. Klassifizieren Sie Layer intersect_wertvoll nach Attribut ffh_typ_text (auf „Klassifizieren“ klicken).
...
4. Filtern Sie Layer intersect_wertvoll mit Hilfe des Abfrageeditors
   mit der Abfrage ffh_typ_nr = 0.
5. Speichern Sie die Auswahl über „Layer > Speichern unter…“
   unter dem Layer-Namen "intersect_wertvoll_ ohne_ffh"
...
9. Nun können Sie eine Karte erstellen, welche
...
11. Ein Beispiel für eine mögliche Kartendarstellung
    sehen Sie auf der folgenden Seite.
