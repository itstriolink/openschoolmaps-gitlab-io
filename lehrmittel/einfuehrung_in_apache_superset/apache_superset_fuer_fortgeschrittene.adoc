...
Die CSV-Dateien `cia_world_factbook_2019.csv` und `country_codes.csv` sollten nun auf deinem Computer vorhanden sein.
...
* Im Formular "CSV to Database configuration" den Tabellennamen auswählen, z.B. `factbook_data`. Dann die CSV-Datei `cia_world_factbook_2019.csv` auswählen, sowie falls nötig die gewünschte Ziel-Datenbank wählen, z.B. `superset-extra-data` (Datenbank muss durch den Superset-Administrator vorbereitet sein).
...
* Ein neues Dashboard kann entweder im Menü _Dashboards_ über den Button "+" rechts erstellt werden oder aber direkt rechts oben mit dem Button "+ New" _> Dashboard_. Es öffnet sich das Formular "Add Dashboard". 
...
Nun ist dein Dashboard publiziert und kann von anderen Benutzer*innen zur Verfügung gestellt werden!
...
Die beiden Tabellen musst du zuerst als Dateien `country_codes.csv` und `cia_world_factbook_2019.csv` herunterladen: 
