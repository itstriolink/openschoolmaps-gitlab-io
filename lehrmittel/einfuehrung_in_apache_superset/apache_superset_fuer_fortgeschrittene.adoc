...
* Dort wo nötig beim entsprechenden Feld (Column Name) auf "Eintrag bearbeiten" klicken, beispielsweise bei der Population.
...
NOTE: Die Zuordnung und Konversion von Datentypen benötigt einiges Knowhow. 
Falls man unsicher ist, sollte man nachfragen.
...
Datum & Zeit (en: Timestamp):: In Superset mit dem Icon "Uhr" dargestellt. Das ist ein "temporaler" d.h. ein zeitlicher Typ mit Datum- und Zeit. Ein zeitlicher Datentyp kann mit oder ohne Zeitzone abgespeichert sein (die Schweiz ist in der Zeitzone "Mitteleuropäische Zeit", MEZ). Diese Datentypen heissen technisch `TIMESTAMP WITH TIMEZONE` und `TIMESTAMP WITHOUT TIMEZONE`. Ein genormte Darstellung eines `TIMESTAMP WITH TIMEZONE` in MEZ sieht z.B. so aus "2019-11-10 12:21:03.000000+01".
...
### Datentyp-Encodings und Geokodierung
