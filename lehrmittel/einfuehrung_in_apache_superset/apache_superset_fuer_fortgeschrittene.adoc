...
Beispiele Datentypen sind Text oder Ganzzahl. 
...
Die Tatsache, dass ein Attribut vom Typ Timestamp ist, wird durch dessen Attributtyp direkt in der Datenbank-Tabelle bestimmt, zusätzlich zu der Kennzeichnung als temporaler Typ, die in Superset noch manuell gesetzt werden muss. (Bei der Checkbox _is temporal_ unter _Columns_ bei der Tabellenansicht).
...
Es gibt auch andere Georeferenzierungs-Systeme, insbesondere die sogenannten *ISO 3166-2 Codes* und die *Geohashes*. 
...
Polyline (WKT):: Damit ist eigentlich ein Encoding gemäss "Well Known Text" (Abk. WKT) gemeint. Beispiel "LineString (47.22311 8.81636, 47.22469, 8.8173)" für die Luftlinie zwischen HSR und Bahnhof Rapperswil. Siehe z.B. https://de.wikipedia.org/wiki/Simple_Feature_Access#Well-known_Text[WKT auf Wikipedia]. 

JSON:: JSON als Abkürzung von "JavaScript Object Notation" ist ein kompaktes Datenformat in einer menschenlesbaren Textform zum Zweck des Datenaustauschs zwischen Anwendungen. 
Hier im Zusammenhang mit Map Charts wird mit JSON eigentlich GeoJSON gemeint. GeoJSON ist ein genormtes Datenformat, um geografische Daten nach der Simple-Feature-Access-Spezifikation zu repräsentieren. GeoJSON kann mit der Website http://geojson.io/[geojson.io] dargestellt und editiert werden. Und mit dem Web-Tool http://geojsonlint.com/[geojsonlint.com] können GeoJSON-Daten geprüft werden.

ISO 3166-2-Code:: Map Charts wie _Country Map_ und _World Map_ können u.a. auch ISO 3166-2-Codes von Ländern und Untergliederungen (Kantone, Departemente, Provinzen) davon verarbeiten. ISO 3166-2 ist Teil der von der Internationalen Organisation für Normung (ISO) veröffentlichten Norm ISO 3166. 
...
Auch normale geografische Namen wie "Rapperswil" oder "Oberseestrasse 10, Rapperswil-Jona" bilden eine Art textuelles Georeferenzierungs-System, wenn auch ein mehrdeutiges. 
Die Interpretation geografischer Namen ist ein aufwändiger Prozess und nennt sich *Geokodierung*. 
Geokodierung ist ein Algorithmus oder Prozess, der eine Beschreibung einer Position (Lage, Ort) – z.B. einer Lat/Lon-Koordinate, eines Geohashs, einer Postadresse oder eines Ortsnamens – in eine Position auf der Erdoberfläche umgewandelt wird. 

Hier als Beispiel einer Geokodierung von "Oberseestrasse 10, Rapperswil-Jona" ein Geokodierungs-Aufruf https://map.search.ch/Rapperswil,Oberseestr.10 .
...
Wobei es durch Verknüpfung mit einer anderen Tabelle immer möglich ist, geografische Namen(-Kürzel) mit Codes zu verknüpfen: 
Siehe dazu das Beispiel im Kapitel "Mit Superset eine Tabelle mit einer anderen verknüpfen".
