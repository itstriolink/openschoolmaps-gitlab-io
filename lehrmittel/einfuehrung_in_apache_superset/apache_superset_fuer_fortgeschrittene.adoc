...
* Eingeben des folgenden SQL-Skripts (dieses sinngemäss anpassen. Zur besseren Lesbarkeit sind hier Tabellennamen in doppelten Anführungszeichen, die SQL-Schlüsselworte in Grossbuchstaben):
----
BEGIN;
...
IMPORTANT: Die SQL-Befehle `BEGIN;` und `COMMIT;` müssen beibehalten werden, um die Datenbank abändern zu können.
