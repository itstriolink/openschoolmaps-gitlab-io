...
Datenanalysen und Visualisierungen verlangen oft eine Verknüpfung von einer Tabellen mit einer anderen. 
Durch diese Verknüpfung werden der ersten Tabelle weitere Datenspalten der weiteren Tabelle(n) in horizontaler Richtung hinzugefügt. 

Diese Verknüpfung geschieht mit der Datenbanksprache SQL und über ein bestimmtes Feld (Attribut, Spalte) der einen Tabelle, das mit einem Feld einer weiteren Tabelle verbunden wird. 
Im Hintergrund muss das Datenbanksystem typischweise ein sogenannter JOIN ausführen.
...
* mit anderer (PostgreSQL-)Database auf demselben Server/Host (d.h. innerhalb der Datenbank von Apache Superset Cloud): tbc.
* mit anderer (PostgreSQL-)Database auf anderem Server/Host (d.h. mit einer Datenbank im Internet ausserhalb der Apache Superset Cloud): tbc. 
...
In der "Einführung in Apache Superset" wurde festgehalten, dass man für einen Chart nur eine einzige Datenquelle (Tabelle) auswählen kann. 
...
Nachfolgend ein beispielhafter Arbeitsablauf in Apache Superset gegeben die Tabelle "Angestellter", welche mit der "Abteilung" verknüpft werden soll und zwar über die "Abteilungs-Id" beim Angestellten, eine Zahl, die auf eine bestimmte Abteilungs-Id verweist:

* Öffnen des Dialogs _SQL Editor_ über das Menü _SQL Lab_.
* Eingeben des folgenden SQL-Skripts (dieses SQL-Skript sinngemäss anpassen, wobei die erste und letzte Zeile (`BEGIN; . . . COMMIT;`) beibehalten werden müssen; Tabellennamen sind hier in Anführungszeichen; die SQL-Schlüsselworte sind hier in Grossbuchstaben zur besseren Lesbarkeit):
----
BEGIN;
CREATE VIEW "angestellter_view" AS 
  SELECT * 
  FROM "angestellter" AS ang 
  JOIN "abteilung" AS abt ON ang.abtid = abt.id 
;
COMMIT;
----
...
* Im Dialog "Import a table definition" als "Table Name" der Namen der soeben erstellen View ergänzen.
* Nun lässt sich die View wie eine Tabelle verwenden (siehe Menü _Source _ > _Tables_), typischerweise zur Erstellung eines Charts.

NOTE: Verknüpfungen wie oben erläutert (mittels einer normalen View) können die Antwortzeiten erheblich verlangsamen. 
Mit weiteren SQL-Skripts kann die Anfrage beschleunigt werden, wofür es jedoch weitere SQL- und PostgreSQL-Kenntnisse nötig sind.

Falls du - wie oben angedeutet - auf weitere Arten eine Tabelle mit einer anderen verknüpfen willst, kontaktiere eine in SQL und PostgreSQL spezialiserte Person. 
