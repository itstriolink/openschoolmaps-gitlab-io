...
* Geo-Datentyp-Rolle "Lines": Das sind Attribute vom Basis-Datentyp Text (ABC in Superset), welche die Datentype-Encodings "JSON" oder "Polylines" haben.

* Geo-Datentyp-Rolle "Polygons": Das sind Attribute vom Basis-Datentyp Text (ABC in Superset), welche die Datentype-Encodings "JSON" oder "ISO 3166-2 codes" haben.
...
Der Name und die Anzeigeeigenschaften können konfiguriert werden. Die Daten werden von einer beliebigen Zeitreihen-Chart geholt.
...
NOTE: Bei vielen Charts gibt es eine 'time_rage'. Sie ist auc 'last week' voreingestellt. Bei den Beispieldaten muss diese meist auf 'no filter' gesetzt werden.
