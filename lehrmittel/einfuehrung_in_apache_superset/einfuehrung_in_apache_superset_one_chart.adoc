...
image::wb_health_population_table.PNG[, 860, 535]
...
* Links befindet sich ein Filter. Damit kann man die Datensätze auf eine Region oder Land beschränken. 
* Rechts befindet sich ein Kreis-Diagramm (Pie Chart), welches die zehn bevölkerungsreichsten Länder und dessen Grösse der Bevölkerung (Metric) anzeigt.

.Das Dashboard mit dem Kreis-Diagramm und dem Filter.
image::Superset_Kurzeinfuehrung_Dashboard.PNG[, 860, 535]
...
Nach dem Hinzufügen einer Komponente kann dessen Grösse durch Anklicken der unteren rechten Ecke angepasst werden. 
