...
In diesem Arbeitsblatt wird nur eine Tabelle verwendet, die Tabelle `wb_health_population` von der Weltbank (https://github.com/apache/incubator-superset/blob/master/superset/examples/countries.md[Quelle], Lizenz CC BY-4.0, Stand ca. 2017, übersetzt etwa "Weltbank-Gesundheit-Population").
...
.Daten von der Schweiz von 1960 - 1976 aus der Tabelle wb_health_population.
image::einfuehrung_in_apache_superset/wb_health_population_table.png[, 860, 535]


== Charts (Diagramme)

Apache Superset bietet zurzeit 40 Charts an. Im Anhang findest du weitere Beispieldiagramme zur 'wb_health_population' Tabelle, die in weiterführenden Informationsblättern vorgestellt werden (Siehe Kapitel "Abschluss").
...
Unter _GROUP BY_ kannst du jetzt deine erste Abfrage definieren, indem du bei _Metric_ eine Spalte wie `SP_POP_TOTL` (Population Total) auswählst und davon die Summe nimmst. 
Dies kannst du tun, indem du auf das Textfeld unter _Metric_ klickst und als Erstes ein _aggregate_ (das sind die Einträge, vor welchen *_AGG_* steht) und dann die entsprechende Spalte wählst.
...
image::Superset_Kurzeinfuehrung_aufg1_loesung.PNG[pdfwidth=85%]
====
...
image::Superset_Kurzeinfuehrung_aufg2_loesung.PNG[pdfwidth=85%]
====
endif::show_solutions[]
...
In diesem Dashboard können nun dein Chart und dein Filter ausgestellt werden.
...
*Aufgabe 3* +
_Erstelle nun ein Dashboard, das der Abbildung 3 entspricht._
...
Um ein Dashboard zu publizieren, musst du lediglich rechts deben dem Namen des Dashboards auf "Draft" drücken, damit "Published" angezeigt wird. Ein publiziertes Dashboard
ist für andere User in der Dashboard-Liste sichtbar (sofern diese User zugriff auf die entsprechenden Datasources haben). Ein Dashboard, welches als "Draft" markiert ist, ist 
zwar nicht für andere User sichtbar (Ausnahme: hochrangige User wie z.B. Admins), dennoch kann man mittels URL direkt drauf zugreifen.
...
Wer mehr über Apache Superset erfahren will, dem sei das ausführlichere Informationsblatt "Einführung in Apache Superset (7 Charts)" und "Apache Superset für Fortgeschrittene" auf OpenSchoolMaps empfohlen.
