...
Wenn man ein Dashboard reproduzieren, oder eine Idee vom Papier auf Superset bringen will, kann es Sinn machen, das Dashboard erst Aufzuteilen, damit das Setzen von
Columns und Rows nicht unnötig kompliziert wird. Hier wird eine Art erläutert, wie man das machen kann. Es ist nicht die effizienteste Art (manche Rows/Columns könnte man
sich sparen), aber für die meisten Anwendungen sollte sie genügen:

Schritt 1:
Man sucht sich auf dem Dashboard alle Linien, mit welchen man das Dashboard aufteilen kann, ohne dass diese Linien durch einen Chart gehen. Dabei dürfen sich diese Linien nicht
kreuzen. D.h. im ersten Schritt sind nur entweder Horizontale oder Vertikale linien erlaubt. In unserem Beispiel (*Ausgangslage*) gibt es nur eine Möglichkeit
...
Diesen Schritt kann man nun wiederholen, bis alle Charts voneinander getrennt sind (*Ebene1 - Ebene4*). Wenn die Aufteilung ähnlich aussieht wie in *EBENE4* kann man mit
...
(Siehe *Aufteilung1*). Als nächstes braucht man in der oberen Row zwei Columns (Siehe *Aufteilung2*). In der Unteren Row braucht man die zwei Columns nicht, da alle Charts
in dieser Row auf einer Gerade liegen und somit platz haben.
Diesen Schritt wiederholt man, für jede Ebene im Diagramm. Wenn man dies gemacht hat, kann man zu Schritt 3 fortfahren.
