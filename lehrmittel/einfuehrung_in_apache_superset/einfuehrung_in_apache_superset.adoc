...
Als erstes sollten Schritt 1 und Schritt 2 (siehe unten) so lange ausgeführt werden, bis alle Charts voneinander getrennt sind (siehe Abb. 23).\
Wenn man so weit ist, sollen die weiteren Schritte der Reihe nach ausgeführt werden.
...
NOTE: Die Linien von zwei aufeinanderfolgenden Schritten sollten immer senkrecht zueinander stehen. Wenn dies nicht der fall ist, hätte man die zwei Schritte zusammen ausführen
können.

=== Schritt 2:
Im 2. Schritt geht es darum, Rows oder Columns ins dashboard zu setzen. Was man ins Dashboard einfügen soll hängt von der vorherigen Ausführung des ersten Schrittes ab.
Wenn man eine waagrechte Linie gezeichnet hat, fügt man zwei Rows ein, wenn man eine senkrechte Linie gezogen hat zwei Columns. Die zwei Rows bzw. Columns kommen immer in das
Feld, welches von der dazugehörigen Linie geteilt wird.

=== Schritt 3:
Entferne alle Columns und Rows, welche nur einen Chart enthalten. In unserem Beispiel sind diese Columns und Rows in Abb. 26 rot markiert. Wenn man diese Charts entfernt hat,
sollte man etwas Ähnliches wie Abb. 28 erhalten.

=== Schritt 4:
Wenn man nun die Charts einführt, erhält man das Endprodukt, welches der Skizze sehr ähnlich sehen sollte (Siehe Abb. 30). Die Charts sind hier dunkelgrau eingezeichnet.
