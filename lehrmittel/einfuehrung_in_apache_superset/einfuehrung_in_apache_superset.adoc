...
* Datasource (Datenquelle): Eine "Source" kann entweder durch das Hochladen einer CSV-Datei oder durch die Erstellung von Views mittels SQL-Abfragen erzeugt werden.
...
Dies kannst du tun, indem du auf das Textfeld unter _Metrics_ klickst und dann als Erstes ein _aggregate_ (das sind die Einträge, vor welchen *_AGG_*, und dann die entsprechende Spalte, wählst.
...
_Group by_ wird genutzt, um ein Total auf ein bestimmtes Attribut aufzuteilen. Wenn man z.B. die Summe der Erdbevölkerung nach Ländernamen gruppiert,
wird jedem Land der entsprechende Teil der Erdbevölkerung zugewiesen.
