...
Hier wird eine Art erläutert, wie man von einer Skizze auf eine Verschachtelung von Rows und Columns kommt, die dieser Skizze entspricht.
...
Um die Verschachtelung zu erhalten, muss man wiederholt Schritt 1 und Schritt 2 ausführen, bis zwischen allen Charts eine Linie gezogen wurde, oder es nicht mehr möglich ist,
weitere Linien zu ziehen. Wenn man keine weiteren Linien mehr ziehen kann und noch nicht alle Charts separiert sind, kann das Dashboard in Superset nicht reproduziert werden.

=== Schritt 1:
Man sucht sich auf der Skizze alle Linien, die das Dashboard teilen, ohne dass sie durch einen Chart gehen. Wenn es mehrere Linien gibt, müssen alle zueinander parallel sein. 
In unserem Beispiel gibt es im ersten Schritt nur eine Möglichkeit dies zu tun (siehe Abb. 15). 
...
Wenn man eine waagrechte Linie gezeichnet hat, fügt man zwei Rows ein, wenn man eine senkrechte Linie gezogen hat zwei Columns. Eine Row bzw. Column soll nur gesetzt werden, wenn sie
mehr als einen Chart umschliesst. Wenn sie nur einen Chart umschliessen würde, wird sie nicht gesetzt. (Die Roten Rows und Columns in den Bildern unten sind Beispiele für
Rows / Columns, die nicht gesetzt werden, da sie nur einen Chart beinhalten.
Stattdessen werden an den entsprechenden Stellen die entsprechenden Charts gesetzt. (Charts sind in den unteren Bildern dunkelgrau markiert)

|===
|*Schritt 1* |*Schritt 2*

a|image::einfuehrung_in_apache_superset/Skizze.PNG[pdfwidth=96%, title="Skizze"]
a|image::einfuehrung_in_apache_superset/Ausgangslage.PNG[pdfwidth=96%, title="Ausgangslage"]


a|image::einfuehrung_in_apache_superset/Aufteilung0.PNG[pdfwidth=96%, title="Die einzige Möglichkeit eine Linie durchs Dashboard zu ziehen"]
