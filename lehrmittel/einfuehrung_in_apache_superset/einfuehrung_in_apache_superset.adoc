...
Um die Aufteilung zu erhalten, muss man lediglich die unteren Schritte mehrmals nacheinander ausführen:

=== Schritt 1:
Man sucht sich auf dem Dashboard alle Linien, die das Dashboard teilen, ohne dass sie durch einen Chart gehen. Diese Linien müssen alle parallel sein. In unserem Beispiel
gibt es im ersten Schritt nur eine Möglichkeit dies zu tun (siehe Abb. 15). 
...
Schritten immer Senkrecht zueinander stehen. Wenn man zwei mal nacheinander dieselbe Richtung erhält, hätte man die zwei Schritte zu einem zusammenfassen können.

=== Schritt 2:
Im 2. Schritt geht es darum, Rows, Columns oder Charts ins Dashboard zu setzen. Was man ins Dashboard einfügen soll hängt von der vorherigen Ausführung des ersten Schrittes ab.
Wenn man eine waagrechte Linie gezeichnet hat, fügt man Rows ein, wenn man eine senkrechte Linie gezogen hat Columns. Eine Row bzw. Column soll nur gesetzt werden, wenn sie
mehr als einen Chart umschliesst. Wenn sie nur einen Chart umschliessen würde, wird sie nicht gesetzt. (Die Roten Rows und Columns in den Bildern oben sind Beispiele für
Rows / Columns, die nicht gesetzt werden, da sie nur einen Chart beinhalten.\
Stattdessen werden an den entsprechenden Stellen die entsprechenden Charts gesetzt. (Charts sind in den oberen Bildern dunkelgrau markiert)

|===
|*Schritt 1* |*Schritt 2*

a|image::einfuehrung_in_apache_superset/Skizze.PNG[pdfwidth=100%, title="Skizze"]
a|image::einfuehrung_in_apache_superset/Ausgangslage.PNG[pdfwidth=100%, title="Ausgangslage"]


a|image::einfuehrung_in_apache_superset/Aufteilung0.PNG[pdfwidth=100%, title="Die einzige Möglichkeit eine Linie durchs Dashboard zu ziehen"]
a|image::einfuehrung_in_apache_superset/Verschachtelung0.PNG[pdfwidth=100%, title="Die waagrechte Linie von Schritt 1 teilt das Dashboard in zwei Rows"]


a|image::einfuehrung_in_apache_superset/Aufteilung1.PNG[pdfwidth=100%, title="In diesem Schritt sind zwei parallele Linien möglich"]
a|image::einfuehrung_in_apache_superset/Verschachtelung1.PNG[pdfwidth=100%, title="Drei Charts können hier direkt eingesetzt werden"]


a|image::einfuehrung_in_apache_superset/Aufteilung2.PNG[pdfwidth=100%, title="Nun ist eine weitere horizontale Linie möglich"]
a|image::einfuehrung_in_apache_superset/Verschachtelung2.PNG[pdfwidth=100%, title="Keine der zwei Rows beinhaltet nur einen Chart, weshalb beide gesetzt werden können"]


a|image::einfuehrung_in_apache_superset/Aufteilung3.PNG[pdfwidth=100%, title="Hier sind wieder zwei senkrechte Linien möglich"]
a|image::einfuehrung_in_apache_superset/Verschachtelung3.PNG[pdfwidth=100%, title="Nur eine Column enthält mehr als einen Chart, weshalb nur eine Column gesetzt wird"]


a|image::einfuehrung_in_apache_superset/Aufteilung4.PNG[pdfwidth=100%, title="Die letzte linie, die gesetzt werden kann"]
a|image::einfuehrung_in_apache_superset/Verschachtelung4.PNG[pdfwidth=100%, title="Hier beinhalten beide Rows nur einen Chart, weshalb keine Rows gesetzt werden"]

a|image::einfuehrung_in_apache_superset/Skizze.PNG[pdfwidth=100%, title="Hier nochmals die Skizze, die das gewünschte Resultat zeigt"]
a|image::einfuehrung_in_apache_superset/Endprodukt.PNG[pdfwidth=100%, title="Hier das Endprodukt, welches der Skizze sehr ähnlich sieht"]
