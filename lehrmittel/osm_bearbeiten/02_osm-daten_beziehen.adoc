...
wenn sich nur für bestimmte Themen oder Lösungen interessiert.
...
Diese Methode empfehlen wir dir zuerst anzuschauen, da sie die effizienteste 
...
Die einen bieten ganze Länder an, die anderen Aussschnitte. 
Auch die Exportformate sind unterschiedlich. 
Einige verlangen, dass man sich registriert und alle sind kostenlos zugänglich 
und werden ohne Gewähr zur Verfügung gestellt. 

Schliesslich noch ein wichtiger Hinweis: Wie in der Einleitung erwähnt, gibt es kein 
einheitliches "GIS"-Datenschema für OSM und darum ist kaum ein Export aus OSM 
gleich wie der andere. Es ist darum wichtig die jeweilige Dokumentation zu 
konsultieren.

Dann gibt es typischerweise noch Nachbearbeitungs-Aufgaben im GIS:

1. Die Geodaten (Layer) auf das Gebiet begrenzen, das dargestellt oder analysiert 
werden soll. 
1. OSM-Objekte die flächenhaft sind möchte man als Punkt-Symbole behandeln und 
darstellen.
1. OSM-Objekte gleicher Art - wie beispielsweise Bäckereien - erscheinen in zwei 
Layern: die einen im Punkt-Layer, die anderen im Polygon-Layer.

Zum ersten Problem verweisen wir an dieser Stelle auf das QGIS-Menü 
"Geoverarbeitungswerkzeuge" und den Algorithmus "Zuschneiden". Dieser Algorithmus 
verlangt die Angabe eines Überlagerungslayers, ein Perimeter, was typischerweise 
eine administrative Grenze ist (also z.B. Uster) _(Tipps, wie man am einfachsten 
eine solche administrative Grenze erhält sind noch in Bearbeitung)_. 

Zu den zwei anderen Problemen gibt es folgende Kapitel unten:
...
nach. Wenn du erfahren möchtest, wie man Objekte filtert, dann springe zum 
Kapitel unten "Mit QGIS Objekte filtern oder regelbasiert darstellen".
====

=== OSM.org

Auf https://openstreetmap.org kann man Daten auch herunter laden.
