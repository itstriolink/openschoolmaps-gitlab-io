...
=== OpenStreetMap-Daten von Geofabrik länderweise beziehen
...
Sobald du das gemacht hast, kannst du zu Tabulator 4 wechseln und den Export erstellen.
...
Nun speichern als GeoJSON... TODO.

Laden GeoJSON in QGIS und dann das GeoJSON als als GeoPackage und gleich auch im Ziel-Koordinatensystem speichern. TODO


TODO => Nach QuickOSM verschieben.

Um diese Daten, also die POIs, ins QGIS zu übertragen, bedienen wir uns einem Tool namens QuickOSM.
...
Um Uster als Hintergrund in QGIS zu setzen, müssen wir ein weiteres Plugin installieren.
Wie vorher das QuickOSM installieren wir jetzt "QuickMapServies".
Sobald es installiert wurde gehen wir unter "Web" auf "QuickMapServies" und wählen dort "OSM" und dann "OSM Standard" aus. 
...
Öffne nun wieder das Plugin QuickOSM unter "Vektor" in der Menüleiste. Dann gib folgende Abfrage ein:

.Bakery-Abfrage
image::osm_editieren/osm-daten_beziehen/quickOSM_bakery.PNG[]

Nachdem die Abfrage ausgeführt wurde, machen wir noch eine zweite, da wir ja auch Supermärkte anzeigen wollen:
...
Jetzt haben wir zwei Punktelayer und zwei Polygonlayer. Da wir aber, wie schon in der ersten Methode mit Overpass-Turbo, nur 
an Punktelayer insteressiert sind, müssen wir noch die Polygonlayer in Punktelayer umwandeln. Dafür gehst du unter "Vektor" 
zu "Geometrie-Werkzeuge" und dann zu "Zentroide...". Wenn du dir nicht mehr sicher bist, schaue weiter oben nochmals nach.
Danach sollte dein QGIS so aussehen, wobei ich die Namen der neuen Layer bereits umbenannt habe:
