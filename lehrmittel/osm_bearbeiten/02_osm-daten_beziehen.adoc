...
Eine Anleitung für Interessierte, für Schülerinnen und Schüler und für Lehrpersonen
...
Die Daten von OpenStreetMap können in unterschiedlichen Datenformaten auf dem 
Rechner lokal gespeichert werden.
...
Wenn du wissen möchtest wie zu Polygon-Layer in Punkte-Layer verwandelst oder 
Objekte filterst, bitte schaue in den Kapiteln "Polygon-Layer mit Punkt-Layer in 
QGIS zusammenführen" oder "Mit QGIS Objekte filtern oder regelbasiert darstellen" 
nach.
...
Wenn du wissen möchtest wie du einen Polygon-Layer in einen Punkte-Layer 
verwandelst oder Objekte filterst, schaue in den Kapiteln 
"Polygon-Layer mit Punkt-Layer in QGIS zusammenführen" oder 
"Mit QGIS Objekte filtern oder regelbasiert darstellen" nach.

== Polygon-Layer mit Punkt-Layer in QGIS zusammenführen

Jetzt haben wir zwei Punkte-Layer und zwei Polygon-Layer. 
Da wir aber nur an einem Punkte-Layer insteressiert sind, müssen wir noch die 
Polygon-Layer in Punkte-Layer umwandeln. 
Dafür gehst du unter "Vektor" zu "Geometrie-Werkzeuge" und dann zu "Zentroide...". 
...
Da wir jetzt vier Punkte-Layer haben, müssen wir diese zusammenführen. 
...
Nachdem man eine Abfrage erstellt oder auf andere Weise OpenStreetMap-Daten 
erhalten hat, möchte man aber vielleicht nicht mit allen Daten arbeiten. 
Wenn also nur eine Teilmenge aller Daten relevant sind, kann man in QGIS die 
Objekte nach selbst definierten Kriterien filtern. 
Eine andere Möglichkeit ist die Daten regelbasiert darzustellen, sodass nur 
jene Daten angezeigt werden, die die Regeln erfüllen.

Im Folgenden schauen wir uns die beiden Methoden an. 
Das Beispiel mit den Brotverkaufsstellen bleibt gleicht. 
Die Brotverkaufsstellen beinhalten sowohl "bakeries" als auch "supermarkets". 
Wir wollen nun aber nur noch "bakeries" in QGIS darstellen.

=== Mit QGIS Objekte filtern

In QGIS hat man zwei Möglichkeiten Objekte zu filtern. 
Entweder man geht bei QGIS im Menü auf "Layer" und dann "Filter..." (während der 
gewünschte Layer ausgewählt ist). 
Oder man macht einen Rechtsklick auf den gewünschten Layer und geht dann auf 
"Filter...". 
Danach öffnet sich ein Fenster mit dem Titel "Abfrageerstellung". 
...
Es gibt noch eine andere Methode, um Objekte in QGIS zu filtern. 
...
(Symbologie) und nicht Teil des Layers wie vorhin. 
Manche sprechen auch von "Daten-getriebender Darstellung".

Dies regelbasierten Darstellung kann man auch dazu verwenden, einzelne Werte 
nicht darzustellen. 
...
Wir sehen, dass unser Layer "Brotverkaufsstellen" nun zwei verschiedene 
Objektgruppen aufweist. 
...
Damit schliessen wir dieses Kapitel ab und überlassen es dir, die Layer und das 
ganze QGIS-Projekt zu erweitern und die Darstellung weiter zu verbessern.