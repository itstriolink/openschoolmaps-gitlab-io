...
.Zeitplanung
Die Bearbeitung dieses Arbeitsblatts verlangt je nach Vorkentnissen unterschiedlich viel Zeit, da man einzelne Abschnitte, die nicht interessieren, überspringen kann.
...
Installiere dazu das Plugin _QuickMapServices_.
Du findest das Plugin unter _Plugins_ -> _Manage and Install Plugins..._ und suche dort nach _QuickMapServices_.
...
Gehe zurück ins QGIS und gehe auf _Add Vector Layer..._ unter _Layer_ -> _Add Layer_.
Wähle die Datei aus, die du gerade heruntergeladen hast und klicke auf _Add_.
...
Nun wollen wir die Punkte filtern nach bestimmten Kriterien.
In unserem Fall filtern wir mal nach Bars.
Natürlich kannst auch nach anderen Punkten filtern.

Um zu Filtern drückst du die rechte Maustaste auf dem Punkte-Layer und wählst _Open Attribute Table_.
Ein Fenster, im Tabellenformat, mit den Daten vom Layer öffnet sich.
Öffne das Tool _Select features using an expression_ mit dem E Symbol oben links im Fenster.
...
Wenn du mit der Selektion zufrieden bist, kannst du auf den Layer rechtsklicken und _Save Selected Features As_ auswählen unter _Export_.
