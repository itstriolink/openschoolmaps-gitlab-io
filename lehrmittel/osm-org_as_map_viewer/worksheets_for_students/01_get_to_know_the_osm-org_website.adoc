...
At the right margin, there are some dark grey buttons with white symbols. Click on the image:osm-org_als_kartenviewer/minus_button.PNG["Minus Button", 20 , 20]- button. What happened?
...
is not found, it is either not in the database or what you have entered is spelled differently in the database. Just change the query and try again!

Try finding your school, you can use the search field or simply move around the map as learned in the section above. If OpenStreetMap fails to query your school by the name or address, you can try including both. 
...
Which city is called "Ciūrichas" in Lithuanian?
...
====
ifdef::show_solutions[]
These are the political boundaries. National borders are shown thicker than cantonal borders and these in turn thicker than district and city borders. 

To sumarise, in terms of thickness:
political > cantonal > district > city
(This information is not shown in the map key, but can be deduced from the map with some geographical knowledge)
